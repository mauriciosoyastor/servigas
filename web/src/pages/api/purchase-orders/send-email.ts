...
import { canSendPurchaseOrderEmail } from "../../../lib/shell/purchase-order-share.ts";
...
export const POST: APIRoute = async ({ cookies, request }) => {
  try {
    ...
    if (!canSendPurchaseOrderEmail(listKey)) {
      throw new BffError("not_found", 404, "Envío no permitido");
    }
    if (!Number.isFinite(id) || id <= 0) {
      throw new BffError("validation_error", 400, "Orden inválida");
    }

    const result = await getBackend().sendPurchaseOrderEmail(
      odooSessionId,
      listKey,
      id
    );
