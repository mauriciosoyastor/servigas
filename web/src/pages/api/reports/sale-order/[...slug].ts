...
export const GET: APIRoute = async ({ cookies, params, url }) => {
  try {
    const slug = Array.isArray(params.slug)
      ? params.slug.join("/")
      : String(params.slug || "");
    const parsed = parseSaleOrderPdfSlug(slug);
    if (!parsed || !canFetchSaleOrderPdf(parsed.listKey)) {
    ...
    const pdf = await getBackend().fetchSaleOrderPdf(
      odooSessionId,
      parsed.listKey,
      parsed.id
    );
