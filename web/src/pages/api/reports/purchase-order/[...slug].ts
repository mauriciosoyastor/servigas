...
export const GET: APIRoute = async ({ cookies, params, url }) => {
  try {
    const slug = Array.isArray(params.slug)
      ? params.slug.join("/")
      : String(params.slug || "");
    const parsed = parsePurchaseOrderPdfSlug(slug);
    if (!parsed || !canFetchPurchaseOrderPdf(parsed.listKey)) {
    ...
    const pdf = await getBackend().fetchPurchaseOrderPdf(
      odooSessionId,
      parsed.listKey,
      parsed.id
    );
